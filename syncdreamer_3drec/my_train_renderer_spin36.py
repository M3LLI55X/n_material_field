...
#     for seg_class in unique_vertex_seg_results:
#         if seg_class != 0:
#             # Generate fields for each segmentation class
#             u = np.zeros([resolution, resolution, resolution], dtype=np.float32)
#             with torch.no_grad():
#                 for xi, xs in enumerate(X):
#                     for yi, ys in enumerate(Y):
...
#                             _, vertex_seg = color_func(pts.cpu().numpy())
#                             vertex_seg_results = np.argmax(vertex_seg, axis=-1)
#                             mask = vertex_seg_results == seg_class
...
#         vertex_colors, _ = color_func(vertices)
